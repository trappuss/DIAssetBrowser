...
bool writeBuffer(const std::string& path, const std::vector<uint8_t>& bytes) {
    // Write to a temporary beside the target and rename into place, so a full
    // disk or a dropped network share leaves the PREVIOUS export intact instead
    // of replacing it with a truncated file that still reports success.
    // (std-only on purpose: this encoder links without Qt.)
    ...
    // rename() will not replace an existing file on Windows; remove first.
